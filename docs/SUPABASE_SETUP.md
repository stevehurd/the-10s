...
- Supabase Auth owns passwordless email-link identities and sessions.
...
4. Store the four values in the development 1Password item described in [LOCAL_SETUP.md](./LOCAL_SETUP.md). Do not add a service-role key.
   Also store `DEVELOPMENT_COMMISSIONER_EMAIL`; the synthetic development seed uses it for the first commissioner profile so passwordless sign-in can link safely.
...
## Configure passwordless email links
...
4. Turn **Confirm email** off. The one-time sign-in link itself proves control of
   the address; disabling this separate signup confirmation keeps first and
   returning sign-ins on the same Magic Link template.
5. Set email-link expiration to 900 seconds.
6. Keep `{{ .ConfirmationURL }}` in the **Magic Link** template. This is
   Supabase's default link behavior; the local version is
   [magic-link.html](../supabase/templates/magic-link.html).
7. Keep anonymous and phone sign-ins disabled.
...
A successful passwordless email link proves control of an email address but does not grant league access. A commissioner must first create the member profile using that exact email. On first verified sign-in, the app links the Supabase Auth UUID to the existing profile. Users without an active pool membership remain unauthorized.
...
After it passes, verify one complete email-link flow and confirm that the signed-in member is linked to the expected pre-created profile.
